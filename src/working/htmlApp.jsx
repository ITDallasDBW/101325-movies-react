...
async function fetchData(searchTerm) {
  ...
  if (data.Response === "False") {
    ...
    if (res.status === 401) {
      ...
      return showMessage(
        `There's no record of <span className="italic">THAT</span> movie`,
        "failure"
      );
    }
  }
  ...
  showMessage(`We found <span className="italic">THAT</span> movie`, "success");
...
function displaySearchTerm(searchData) {
  resultsMessage.innerHTML = `<h3 className="results__message">What we found for <span className="glow italic">${searchData}</span>:</h3>`;
}
